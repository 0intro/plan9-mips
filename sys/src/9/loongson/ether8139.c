...
rtl8139ifstat(Ether* edev, void* a, long n, ulong offset)
{
	...
	l += snprint(p+l, READSTR-l, "Tsd0-3 %#8.8lux %#8.8lux %#8.8lux %#8.8lux\n",
		csr32r(ctlr, Tsd0), csr32r(ctlr, Tsd0+4),
		csr32r(ctlr, Tsd0+8), csr32r(ctlr, Tsd0+12));
	l += snprint(p+l, READSTR-l, "Cr %#2.2ux\n", csr8r(ctlr, Cr));
	l += snprint(p+l, READSTR-l, "Tcr %#8.8lux\n", csr32r(ctlr, Tcr));
	l += snprint(p+l, READSTR-1, "Rcr %#8.8lux\n", csr32r(ctlr, Rcr));
	l += snprint(p+l, READSTR-1, "Rbstart %#8.8lux\n", csr32r(ctlr, Rbstart));
	l += snprint(p+l, READSTR-1, "Capr %#4.4ux\n", csr16r(ctlr, Capr));
	l += snprint(p+l, READSTR-1, "Cbr %#4.4ux\n", csr16r(ctlr, Cbr));
...
rtl8139txstart(Ether* edev)
{
	...
	while(ctlr->ntd < Ntd){
		...
		td = &ctlr->td[ctlr->tdh];
		memmove(KSEG1ADDR(td->data), bp->rp, size);
		freeb(bp);
