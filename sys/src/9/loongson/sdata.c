...
static SDev*
ataprobe(int cmdport, int ctlport, int irq, int subirq)
{
	Ctlr* ctlr;
	SDev *sdev;
	Drive *drive;
...
atastat(SDev *sdev, char *p, char *e)
{
	...
	return seprint(p, e, "%s ata port %X ctl %X irq %d.%d "
		"intr-ok %lud intr-busy %lud intr-nil-drive %lud\n",
		sdev->name, ctlr->cmdport, ctlr->ctlport, ctlr->irq, ctlr->subirq,
...
atadmasetup(Drive* drive, int len)
{
	...
	if(i == Nprd)
		(prd-1)->count |= PrdEOT;
	dcflush(ctlr->prdt, i*sizeof(Prd));
...
atageniostart(Drive* drive, uvlong lba)
{
	...
	case Crd:
		dcinvalid(drive->data, drive->count*drive->secsize);
		atadmastart(ctlr, drive->write);
		break;

	case Cwd:
		dcflush(drive->data, drive->count*drive->secsize);
		atadmastart(ctlr, drive->write);
		break;
	}
	iunlock(ctlr);
...
atapnp(void)
{
	...
	if(sdev = ataprobe(0x1F0, 0x3F4, IL8259, IrqATA0)){
		head = tail = sdev;
		legacy[0] = sdev;
	}
	if(sdev = ataprobe(0x170, 0x374, IL8259, IrqATA1)){
		if(head != nil)
			tail->next = sdev;
		else
			head = sdev;
...
{
	return ataprobe(port, port+0x204, IL8259, irq);
}
...
ataenable(SDev* sdev)
{
	...
	intrenable(ctlr->irq, atainterrupt, ctlr, ctlr->subirq);
...
atadisable(SDev *sdev)
{
	...
	if (ctlr->idisable)
		ctlr->idisable(ctlr);
	snprint(name, sizeof(name), "%s (%s)", sdev->name, sdev->ifc->name);
//	intrdisable(ctlr->irq, atainterrupt, ctlr, ctlr->subirq); // XXX
	if (ctlr->bmiba) {
		if (ctlr->pcidev)
