/*
 *	radeon frame buffer
 *	currently use all PMON defaults
 */
#include "u.h"
#include "../port/lib.h"
#include "mem.h"
...
screensize(void)
{
	...
	if(p == nil || getfields(p, f, nelem(f), 0, "x") != nelem(f) ||
	    	...
	    	(depth = atoi(f[2])) <= 0) {
		width = screenwid? screenwid: Wid;
		height = screenht? screenht: Ht;
		depth = screendepth? screendepth: Depth;
	}
