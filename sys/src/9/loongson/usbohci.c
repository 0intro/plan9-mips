...
isodtdinit(Ep *ep, Isoio *iso, Td *td)
{
	...
	td->nbytes = size;
	memset(KSEG1ADDR(bp->wp), 0, size);	/* in case we don't fill it on time */
...
epgettd(Ep *ep, Qio *io, Td **dtdp, int flags, void *a, int count)
{
	...
	if(count > 0){
		...
		if(a != nil){
			...
			memmove(KSEG1ADDR(bp->wp), a, count);
		}
		bp->wp += count;
	}
	td->nbytes = count;
	...
	for(td = td0; td != ltd; td = ntd){
		...
		if(last == 0 && tderrs(td) == Tdok){
			...
			if(c != nil && tdtok(td) == Tdtokin && n > 0){
				memmove(c, KSEG1ADDR(td->bp->rp), n);
				c += n;
			}
		}
		last |= td->last;
	...
	case Tctl:
		...
		if(cio->ndata < 0)
			...
			if(count > cio->ndata)
				count = cio->ndata;
			if(count > 0)
				memmove(a, KSEG1ADDR(cio->data), count);
...
putsamples(Ctlr *ctlr, Ep *ep, Isoio *iso, uchar *b, long count)
{
	...
	memmove(KSEG1ADDR(td->bp->wp), b, n);
	td->bp->wp += n;
...
//		}
		...
		ctlr->pcidev = p;
		ctlr->ohci = KSEG1ADDR(PCIMEMADDR(mem));
