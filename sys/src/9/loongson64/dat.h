...
struct Mach
{
	...
	int		machno;			/* physical id of processor FIRST @ 0 */
	Softtlb*stb;			/* Software tlb simulation SECOND @ 8 */
	Proc*	proc;			/* process on this processor THIRD @ 16 */
	uintptr	splpc;			/* pc that called splhi() FOURTH @ 24 */
	ulong	tlbfault;		/* # of tlb faults FIFTH @ 32 */
	ulong	ktlbfault;		/* @ 36 */
	ulong	utlbfault;		/* @ 40 */
...
struct KMap
{
	...
	Page*	pg;
	uintptr	pc;			/* of caller to kmap() */
};

#define	VA(k)		((k)->virt)
#define PPN(x)		((uintptr)(x)>>6)		/* PPN in TLBPHYS0-1 */
...
typedef vlong regint;		/* register sized int, ensure single instr'n */
