...
{	.regs	= &i8250ctlr[0],
	.name	= "cons",
	.freq	= 1843200,	/* Not used, we use the global i8250freq */
	.phys	= &i8250physuart,
	.console= 1,
	.next	= nil, },
};
...
i8250enable(Uart* uart, int ie)
{
	...
	if(ie){
		if(ctlr->iena == 0 && !ctlr->poll){
			intrenable(ctlr->irq, i8250interrupt, uart, 0);
			ctlr->iena = 1;
		}
...
i8250putc(Uart* uart, int c)
{
	...
	if (!normalprint) {		/* too early; use brute force */
		s = splhi();
		while (!(((uchar*)PHYSCONS)[Lsr] & Thre))
			;
		((uchar*)PHYSCONS)[Thr] = (uchar)c;
		splx(s);
		return;
	}
